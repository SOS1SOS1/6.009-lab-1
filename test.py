...
def test_inverted_2():
    im = lab.load_image(os.path.join(TEST_DIRECTORY, 'test_images', 'centered_pixel.png'))
    result = lab.inverted({'height': 1, 'width': 4, 'pixels': [23, 65, 141, 216]})
    expected = {
        'height': 1,
        'width': 4,
        'pixels': [232, 190, 114, 39],
    }
    compare_images(result, expected)
...
def test_correlation_identity():
    ...
    result = lab.correlate(im, identity_kernel)
...
def test_blurred_black_image():
    ...
    result = lab.blurred(black_img, 3)
    expected = black_img
    compare_images(result, expected)
    result = lab.blurred(black_img, 5)
    compare_images(result, expected)
